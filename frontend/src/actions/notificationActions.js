...
export const getNotifications = () => async (dispatch) => {
    try {
        ...
        let link = `http://127.0.0.1:8000/api/v1/notifications`
...
export const getNotificationDetails = (id) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`http://127.0.0.1:8000/api/v1/notification/${id}`)
        dispatch({
...
export const updateNotification = (id) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.put(`http://127.0.0.1:8000/api/v1/admin/notification/${id}` , config)
        dispatch({
...
export const deleteNotification = (id) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.delete(`http://127.0.0.1:8000/api/v1/admin/notification/${id}`)
        dispatch({
