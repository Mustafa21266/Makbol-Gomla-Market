...
export const getProducts = (keyword = '',currentPage = 1, price, category, subcategory, rating = 0) => async (dispatch) => {
    try {
        ...
        let link = `http://localhost:8000/api/v1/products?keyword=${keyword}&page=${currentPage}&price[lte]=${price[1]}&price[gte]=${price[0]}&ratings[gte]=${rating}`
        if(category){
            link = `http://localhost:8000/api/v1/products?keyword=${keyword}&page=${currentPage}&price[lte]=${price[1]}&price[gte]=${price[0]}&ratings[gte]=${rating}&category=${category}&subcategory=${subcategory}`
        }
...
export const deleteReview = (id, productId) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.delete(`http://localhost:8000/api/v1/reviews?id=${id}&productId=${productId}`)
        dispatch({
