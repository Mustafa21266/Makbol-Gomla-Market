    ...
    const setOrders = () => {
        ...
        orders.forEach(order => {
            data.rows = data.rows.concat ({
                ...
                status: order.orderStatus && String(order.orderStatus).includes('Delivered') ? <p style={{color: 'green', textAlign: 'center'}}>تم التوصيل</p> : String(order.orderStatus).includes('Delivered') ? <p style={{color: 'red', textAlign: 'center'}}>تحت التأكيد</p> : <p style={{color: 'red', textAlign: 'center'}}>جاري التوصيل</p> ,
                actions: 
                <Fragment>
                    <div className="row">
