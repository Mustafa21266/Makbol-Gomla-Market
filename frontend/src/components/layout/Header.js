                ...
                <div id="parent3" className="dropdown-menu drop-carta dropdown-menu-left animate__animated animate__fadeIn"  style={{position: 'absolute', right: -120 ,top: 35,padding: '25px 15px',width: '350px'}} aria-labelledby="dropDownMenuButtonTwo">
                  {cartItems && cartItems.map(item => (
                    <Fragment key={item._id}>
                      <Link  onClick={(e) => document.getElementById('parent3').style.display = "none" }  to={`/product/${item.product}`} className="dropdown-item">
                  ...
                  <Link onClick={(e) => document.getElementById('parent1').style.display = "none" } to={'/cart'} className="btn w-100"  style={{backgroundColor:'#178a53', color:"white"}} >إذهب إلى سلة التسوق</Link>
                  </div>
                  </div> 
          </div> 
</div>
